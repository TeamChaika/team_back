"""Один отчёт по денежным балансам: проверка ответа и сохранение исходного JSON."""
...
from app.schemas.iiko_balances import (
    CounteragentBalance,
    CounteragentBalancesQuery,
...
def read_counteragent_balances(
    path: Path, query: CounteragentBalancesQuery
    ...
    try:
        ...
        rows = [CounteragentBalance.model_validate(item) for item in payload]
        for row in rows:
            for field, ids in [
                ("account_id", query.account_id),
                ("counteragent_id", query.counteragent_id),
        ...
        raise IikoError(
            "iiko_balances_invalid_response",
            "Ответ балансов повреждён или не соответствует запрошенным фильтрам.",
        ) from None


class IikoBalancesService:
    def __init__(
        ...
        self._directory = (
            directory if directory is not None else BACKEND_DIR / ".local/counteragent-balances"
        )

    async def get(self, query: CounteragentBalancesQuery) -> CounteragentBalancesResponse:
        snapshot_id = uuid4()
        ...
        try:
            ...
            download = await self._auth.download_counteragent_balances(
                part_file,
                timestamp=query.timestamp,
                account_ids=query.account_id,
                counteragent_ids=query.counteragent_id,
                department_ids=query.department_id,
            )
            received_at = datetime.now(UTC)
            rows = await asyncio.to_thread(read_counteragent_balances, part_file, query)
            ...
            metadata["source_endpoint"] = "v2/reports/balance/counteragents"
            ...
            raise IikoError(
                "iiko_balances_storage_error",
                "Не удалось сохранить отчёт балансов локально.",
                status_code=500,
            ) from None
        finally:
            for path in [part_file] if saved else [part_file, meta_file]:
                try:
                    ...
                    logger.warning("Не удалось удалить незавершённый файл отчёта балансов.")
