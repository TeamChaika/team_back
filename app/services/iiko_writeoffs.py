"""Акты списания: проверка JSON-обёртки, документов и сохранение исходного ответа."""
...
from app.schemas.iiko_writeoffs import (
    WriteoffExport,
    WriteoffsQuery,
    WriteoffsResponse,
)
...
def read_writeoffs(path: Path, query: WriteoffsQuery) -> WriteoffExport:
    try:
        payload = json.loads(
        ...
        if payload["result"] != "SUCCESS" or payload["errors"]:
            raise IikoError(
                "iiko_writeoffs_export_failed", "iiko сообщил об ошибке выгрузки актов списания."
            )
        export = WriteoffExport.model_validate(payload)
        ids = set()
        ...
        raise IikoError(
            ...
            "Ответ актов списания повреждён или не соответствует запросу.",
        ) from None


class IikoWriteoffsService:
    def __init__(
        ...
        self._directory = directory if directory is not None else BACKEND_DIR / ".local/writeoffs"

    async def get(self, query: WriteoffsQuery) -> WriteoffsResponse:
        snapshot_id = uuid4()
        ...
        try:
            ...
            download = await self._auth.download_writeoffs(
                part_file,
            ...
            export = await asyncio.to_thread(read_writeoffs, part_file, query)
            response = WriteoffsResponse(
            ...
            metadata["source_endpoint"] = "v2/documents/writeoff"
            ...
            raise IikoError(
                "iiko_writeoffs_storage_error",
                "Не удалось сохранить акты списания локально.",
                status_code=500,
            ) from None
        finally:
            for path in [part_file] if saved else [part_file, meta_file]:
                try:
                    ...
                    logger.warning("Не удалось удалить незавершённый файл актов списания.")
