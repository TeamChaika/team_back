...
class Source:
    ...
    def __call__(self, request):
        ...
        assert request.url.path == "/resto/api/v2/documents/internalTransfer"
...
def test_precision_cost_nulls_statuses_dates_and_revision_are_preserved(tmp_path):
    ...
    with TestClient(app(source, tmp_path)) as c:
        ...
        assert metadata["source_endpoint"] == "v2/documents/internalTransfer"
...
def test_invalid_exports_are_not_published_or_leaked(tmp_path, payload):
    with TestClient(app(Source(payload), tmp_path)) as c:
        r = c.get(BASE, params=PARAMS)
        assert (
            r.status_code == 502 and r.json()["error"]["code"] == "iiko_transfers_invalid_response"
        )
...
def test_http_200_business_errors_do_not_publish_revision(tmp_path, overrides):
    with TestClient(app(Source(body(**overrides)), tmp_path)) as c:
        r = c.get(BASE, params=PARAMS)
        assert r.status_code == 502 and r.json()["error"]["code"] == "iiko_transfers_export_failed"
        assert "private-error" not in r.text and not list(tmp_path.iterdir())
...
def test_size_and_disk_failure_clean_unfinished_files(tmp_path, monkeypatch):
    with TestClient(app(Source(), tmp_path, config(iiko_transfers_max_response_bytes=10))) as c:
        assert c.get(BASE, params=PARAMS).json()["error"]["code"] == "iiko_transfers_too_large"
        assert not list(tmp_path.iterdir())
...
def test_only_401_is_retried_once(tmp_path, statuses):
    ...
    with TestClient(app(source, tmp_path)) as c:
        ...
        assert sum(r.url.path.endswith("/internalTransfer") for r in source.requests) == len(
            statuses
        )
...
def test_timeout_keeps_session_available_for_logout(tmp_path):
    async def handler(request):
        if request.url.path.endswith("/internalTransfer"):
            await asyncio.sleep(1)
        return httpx.Response(200, text=TOKEN)

    with TestClient(app(handler, tmp_path, config(iiko_transfers_timeout_seconds=0.01))) as c:
        assert c.get(BASE, params=PARAMS).status_code == 504
        assert not list(tmp_path.iterdir())
        assert c.post("/api/v1/iiko/logout").json()["state"] == "logged_out"


def test_transfers_employees_and_logout_share_one_request_lock(tmp_path):
    async def scenario():
        active = maximum = 0
        paths = []
        ...
        try:
            await asyncio.gather(
                auth.download_transfers(
                    tmp_path / "transfers.json",
            ...
            assert maximum == 1 and paths == ["auth", "internalTransfer", "employees", "logout"]
        finally:
            await auth.aclose()
...
def test_openapi_exposes_only_list_reading_and_public_field_names(tmp_path):
    ...
    with TestClient(app(source, tmp_path)) as c:
        ...
        fields = schema["components"]["schemas"]["TransferDocumentItem"]["properties"]
        assert (
