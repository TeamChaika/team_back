...
- `app/web/indicators.py` — дневные KPI: `/api/indicators/filters`, `POST /api/indicators/query`, `POST /api/indicators/options/{field}`. Базовый запрос читает архив и общий кеш сегодняшних продаж. Дополнительные фильтры или `direct: true` используют последовательные OLAP-запросы с кешем на 5 минут; список полей фиксирован сервером, доступ ограничен ресторанами пользователя. Незагруженные показатели возвращаются как `null`.
